...
namespace Magenest\CustomInventoryReservation\Rewrite\Magento\InventorySales\Model\Adminhtml;
...
class GetProductSalableQty extends \Magento\InventorySales\Model\GetProductSalableQty
{
    ...
    public function execute(string $sku, int $stockId): float
    {
        ...
        foreach ($stockItemData as $data) {
            $reservationQtyArray = json_decode($this->getReservationsQuantityV2->execute($sku, $stockId, $data['area_code']), true);
            if ($data[\Magento\InventorySalesApi\Model\GetStockItemDataInterface::IS_SALABLE] == 1) {
        ...
        $productQtyInStockFloat = 0;
        foreach ($productQtyInStock as $key => $one) {
            $productQtyInStockFloat += $one;
        }
